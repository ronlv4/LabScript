...
#         # this is the step size for the numerical calculation of the df/dx = last value in x (x[-1]) - first value in x (x[0])/10000
#
...
#         # the derivative df/dx at point x is taken as [f(x+h)-f(x)]/h
#         chi2 = sum(((self.y - self.ym) ** 2) / (self.dy ** 2 + (df * self.dx) ** 2))
#         # chi2 is now Sum of: (f(x)-y)^2/(uncert_y^2+(df/dx*uncert_x)^2)
#         return chi2
#
...
# just note that in practice it will be used for the same thing but in "coding" terms this is a different variable x than x in the class def above

# Now we create a regression object (from the class defined above) with our example data and function
efvtest = EffVarChi2Reg(linear_fun, measured_x, measured_y, x_uncertainties, y_uncertainties)
# the syntax for EffVarChi2Reg is (function, x,y,dx,dy)
...
efvtest.show(efopt, goodness_loc=4, x_title="X_data", y_title="Y_data")
